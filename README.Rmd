...
Download the dataset, `Ames_data.csv`, from the Resouces page. The dataset has 2930 rows (i.e., houses) and 83 columns. 
...
Download `project1_testIDs.dat` from the Resouces page. This file contains 879 rows and 10 columns, which will be used to generate 10 sets of training/test splits from `Ames_data.csv`. Each column contains the 879 row-numbers of a test data.  

How to generate a split of training and test using the j-th column of `project1_testIDs.dat`?
...
```
Save the training and test as csv files. In particular, the test data are saved as two seperate two files: one contains just the feature vectors and the other one contains the response column. For your remaining analysis, you can read the training and test data from the csv files. 
...
* <font color="red">R/Python code</font> in a single file named **mymain.R** (or **mymain.py**) that takes a training data and a test data as input and outputs two submission files (the format of the submission file is given below). **No zip file; no R markdown file**.
  ...
  Your mymain.R should look like the following. **Note** that you have to pre-process your training and test data separately. 
...
* train.csv: about 70% of the whole data "Ames_data.csv" with exactly the same 83 columns;
* test.csv: the remaining of the whole data without the last column "Sale_Price", i.e., it has 82 columns.
